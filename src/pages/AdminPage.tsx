...
const AdminPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!authLoading && (!user || !user.isHighStaff)) {
      navigate('/');
    }
  ...
  const handleApprove = async (id: string, comment: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: comment ? `Комментарий: ${comment}` : 'Участник повышен',
      });
    } catch (error) {
      toast({
        title: 'Ошибка',
  ...
  const handleReject = async (id: string, comment: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: comment ? `Причина: ${comment}` : 'Заявка отклонена',
        variant: 'destructive',
      });
    } catch (error) {
      toast({
            ...
            <TabsList className="bg-muted/50 border border-border/50">
              ...
              <TabsTrigger value="promotions" className="gap-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                ...
                {promotionRequests.length > 0 && (
                  <Badge className="ml-1 h-5 px-1.5 text-xs bg-warning text-warning-foreground">
            ...
            <div className="space-y-6 overflow-y-auto flex-1">
              ...
              <div className="flex items-center gap-4">
                <Avatar className="w-16 h-16 border-2 border-primary/30">
                  <AvatarImage src={selectedMember.avatar || undefined} alt={selectedMember.displayName} />
                  <AvatarFallback className="bg-muted text-muted-foreground font-display">
                    {selectedMember.displayName.slice(0, 2).toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <h3 className="font-display text-xl font-semibold">{selectedMember.displayName}</h3>
