...
import { LogOut, Shield, FileText, TrendingUp, Send, Crown, Loader2, Camera, Clock, Trophy } from 'lucide-react';
import { toast } from '@/hooks/use-toast';
import { cn } from '@/lib/utils';

// ---- Модалка з вимогами для підвищення ----
...
const DashboardPage: React.FC = () => {
  ...
  const { requestPromotion, hasPromotionRequest } = useMembers();
  ...
  // Для модалки підвищення
  const [showPromotionModal, setShowPromotionModal] = useState(false);
  const [selectedRank, setSelectedRank] = useState<'Test' | 'Main' | null>(null);

  useEffect(() => {
    if (!authLoading && !user) navigate('/');
    else if (!authLoading && user && !user.hasAccess) navigate('/no-access');
  }, [authLoading, user, navigate]);

  useEffect(() => {
    if (!user) return;
    setHasRequestedPromotion(hasPromotionRequest(user.id));
  }, [user, hasPromotionRequest]);
  ...
  const handleAddReport = async (report: { type: 'video' | 'screenshot'; url: string; description: string }) => {
    try { await addReport(report); } 
    catch { toast({ title: 'Ошибка', description: 'Не удалось загрузить отчёт.', variant: 'destructive' }); }
  };
      ...
      <div className="relative z-10">
        ...
        <header className="border-b border-border/50 bg-card/50 backdrop-blur-xl sticky top-0 z-50">
          <div className="container mx-auto px-4 h-16 flex items-center justify-between">
            <h1 className="font-display text-xl font-bold gradient-text">LITHIUM CLNX</h1>
            <div className="flex items-center gap-4">
              {user.isAdmin && (
                <Button variant="secondary" size="sm" className="gap-2" onClick={() => navigate('/admin')}>
                ...
                <div className="flex-1 text-center md:text-left">
                  <div className="flex flex-col md:flex-row items-center gap-3 mb-2">
                    ...
                    <Badge className={cn("border", roleColors[user.highestRole] || roleColors['Newbie'])}>
                      <Crown className="w-3 h-3 mr-1" />{user.highestRole}
                    </Badge>
                  </div>
                  <p className="text-muted-foreground">@{user.username}</p>
                  <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-4">
                    <div className="flex items-center gap-2 text-sm"><FileText className="w-4 h-4 text-primary" /> <span>{reports.length} отчётов</span></div>
                    <div className="flex items-center gap-2 text-sm"><TrendingUp className="w-4 h-4 text-success" /> <span>Ранг: {user.rank}</span></div>
                  </div>
                </div>

                <div className="flex flex-col gap-3">
                  <UploadReportDialog onSubmit={handleAddReport} />
                  {user.rank && user.rank !== 'Main' && user.rank !== 'High Staff' && (
                    <Button 
                      variant="outline" 
                      size="lg" 
                      className="gap-2"
                    ...
                    >
                      {isSubmittingPromotion ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
        ...
        {/* Rank Progress з клікабельними кружочками */}
        <Card className="bg-card/50 backdrop-blur-xl border-border/50 mb-8">
          <CardHeader>
            <CardTitle className="font-display flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-primary" /> Путь развития
            </CardTitle>
            <CardDescription>Кликните на кружок ранга, чтобы увидеть требования</CardDescription>
          </CardHeader>
          <CardContent className="relative">
            <RankProgress currentRank={user.rank} daysUntilNextRank={user.daysUntilNextRank} />
          {/* Кнопка для Test */}
          <div
            onClick={() => { setSelectedRank('Test'); setShowPromotionModal(true); }}
            className="absolute left-[50%] top-[21%] w-16 h-16 -translate-x-1/2 -translate-y-1/2 cursor-pointer z-20"
          />

          <div
            onClick={() => { setSelectedRank('Main'); setShowPromotionModal(true); }}
            className="absolute left-[93.5%] top-[21%] w-16 h-16 -translate-x-1/2 -translate-y-1/2 cursor-pointer z-20"
          />
          </CardContent>
        </Card>
          ...
          <Card className="bg-card/50 backdrop-blur-xl border-border/50">
            <CardHeader>
              <CardTitle className="font-display flex items-center gap-2"><FileText className="w-5 h-5 text-primary" /> Мои отчёты</CardTitle>
              <CardDescription>История загруженных отчётов</CardDescription>
            </CardHeader>
            <CardContent>
              {reportsLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground/70 mt-1">Загрузите первый отчёт, чтобы начать</p>
                </div>
              ) : (
                <div className="space-y-3">{reports.map(report => <ReportCard key={report.id} report={report} />)}</div>
              )}
            </CardContent>
          </Card>
        </main>
